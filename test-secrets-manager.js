...
async function testSecretsManager() {
  try {
    ...
    const poolTestResult = await executeQuery('SELECT DATABASE() as current_db');
    ...
    if (error.message.includes('Secret not found')) {
      console.log('\n💡 Setup Instructions:');
      console.log('1. Create a secret in AWS Secrets Manager with your database credentials');
      ...
      console.log('   }');
      console.log('3. Set the DB_SECRET_ID environment variable to your secret ARN or name');
      console.log('4. Ensure your Lambda has permissions to access the secret');
    }
    
    if (error.message.includes('Access denied')) {
      console.log('\n💡 Permission Issue:');
      console.log('- Ensure your Lambda execution role has secretsmanager:GetSecretValue permission');
...
function showEnvironmentInfo() {
  ...
  console.log('- DB_SECRET_ID:', process.env.DB_SECRET_ID || 'not set (will use env vars)');
